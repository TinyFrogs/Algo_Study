...
public class BOJ_19236 {
	...
	static class Fish {
		...
		Fish(int idx, int x, int y, int dir, int alive) {
			this.idx = idx;
			this.x = x;
			this.y = y;
			this.dir = dir;
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 4; j++) {
				int idx = Integer.parseInt(st.nextToken()); // 물고기 번호
				int dir = Integer.parseInt(st.nextToken()) - 1; // 물고기 방향
				fish[idx] = new Fish(idx, i, j, dir, 1);
				map[i][j] = idx; //map에 물고기 번호 저장
			}
		}
	...
	public static void dfs(int sx, int sy, int sd, int eat) {
		...
		for (int i = 1; i <= 16; i++)
			tempFish[i] = new Fish(fish[i].idx, fish[i].x, fish[i].y, fish[i].dir, fish[i].alive);
		...
		for (int i = 1; i <= 16; i++)
			fish[i] = new Fish(tempFish[i].idx, tempFish[i].x, tempFish[i].y, tempFish[i].dir, tempFish[i].alive);
	}
			...
			while (cnt < 8) { //이동할 수 있는 위치를 찾을때까지 45도 방향 바꾸며 반복
				...
				if (nx >= 0 && nx < 4 && ny >= 0 && ny < 4 && map[nx][ny] != -1) {
					if (map[nx][ny] == 0) { //이동할 위치가 빈칸일 경우
						...
						// 바꿀 물고기 위치 변경
						int changeFish = fish[map[nx][ny]].idx;
